            ...
            <div class="form-group fieldcontain ${hasErrors(bean: depositInstance, field: 'glLink', 'has-error')} ">
                ...
                <div class="col-sm-8">
                    <g:if test="${depositInstance?.typeId == 1}">
                        <g:select id="glLink" name="glLink.id" from="${icbs.gl.CfgAcctGlTemplate.findAllByType("1")}" optionKey="id" optionValue ="description" value="${depositInstance?.glLink?.id}" class="form-control" noSelection="['null': '']" />
                    </g:if>
                    
                    <g:if test="${depositInstance?.typeId == 2}">
                        <g:select id="glLink" name="glLink.id" from="${icbs.gl.CfgAcctGlTemplate.findAllByType("2")}" optionKey="id" optionValue ="description" value="${depositInstance?.glLink?.id}" class="form-control" noSelection="['null': '']" />
                    </g:if>
                    
                    <g:if test="${depositInstance?.typeId == 3}">
                        <g:select id="glLink" name="glLink.id" from="${icbs.gl.CfgAcctGlTemplate.findAllByType("3")}" optionKey="id" optionValue ="description" value="${depositInstance?.glLink?.id}" class="form-control" noSelection="['null': '']" />
                    </g:if>
                    
                    <g:if test="${depositInstance?.typeId == 4}">
                        <g:select id="glLink" name="glLink.id" from="${icbs.gl.CfgAcctGlTemplate.findAllByType("4")}" optionKey="id" optionValue ="description" value="${depositInstance?.glLink?.id}" class="form-control" noSelection="['null': '']" />
                    </g:if>
                    
                    <g:if test="${depositInstance?.typeId == 5}">
                        <g:select id="glLink" name="glLink.id" from="${icbs.gl.CfgAcctGlTemplate.findAllByType("5")}" optionKey="id" optionValue ="description" value="${depositInstance?.glLink?.id}" class="form-control" noSelection="['null': '']" />
                    </g:if>
             ...
             <div class="form-group fieldcontain ${hasErrors(bean: depositInstance, field: 'depositClassification', 'has-error')} ">
            <label class="control-label col-sm-4"for="depositClassification">
                    <g:message code="depositInstance.depositClassification.label" default="Deposit Classification" />
            </label>
            <div class="col-sm-8"><g:select id="depositClassification" name="depositClassification.id" from="${icbs.lov.DepositClassification.findAllByStatus(true)}" optionKey="id" optionValue ="description" value="${depositInstance?.depositClassification?.id}" class="form-control" noSelection="['null':'']"/>
                <g:hasErrors bean="${depositInstance?.type?.description}" field="depositClassification">
                    <div class="controls">
                        <span class="help-block">
                            <g:eachError bean="${depositInstance?.type?.description}" field="depositClassification">
                                <g:message error="${it}" /><br/>
                            </g:eachError>
                        </span>
                    </div>
